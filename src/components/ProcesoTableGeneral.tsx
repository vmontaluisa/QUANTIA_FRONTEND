...
const ProcesoTableGeneral: React.FC<ProcesoTableProps> = ({ title, subtitle, type, lineas, data,exportar }) => {
          ...
          <Table stickyHeader size="small">
            <TableHead>
              <TableRow>
                ...
                >
                  <span style={{ textDecoration: 'underline' }}><strong>TÉRMINOS</strong></span> {orderBy === 'P2_terminos' && (orderDirection === 'asc' ? ' ▲' : ' ▼')}
                </TableCell>
                <TableCell
            ...
            <TableBody>
              {paginatedData.map((row, index) => (
                <TableRow key={index} sx={{ backgroundColor: index % 2 === 0 ? '#fafafa' : 'white' }}>
                  ...
                  <TableCell sx={{ textAlign: 'justify', px: 1, py: 0.5, textTransform: 'uppercase', lineHeight: '1.1' }}>
                    {row.P2_terminos}
                    {row.P1 === 0 && row.P2 > 0 && (
                      <Tooltip
                        title="Este proceso no cumple con los criterios CPC de su empresa"
                        arrow
                        componentsProps={{
                          tooltip: {
                            sx: {
                      ...
                      >
                        <NotificationImportantIcon fontSize="small" sx={{ ml: 1, color: 'red' }} />
                      </Tooltip>
                    )}
...
export default ProcesoTableGeneral;
